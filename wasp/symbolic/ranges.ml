...
let old_range_list (lst : int list) : string =
  ...
  let filelines = File.lines_of "useless_lines.txt" in
  Enum.iter (fun l -> if l<>"" then (useless_lines := !useless_lines @ [(int_of_string l)])) filelines;
  ...
  let sorted = List.sort_unique compare lst in
  
  (* Iterate function by function *)
  let functions = File.lines_of "functions.txt" in
  let ranges = ref "" in
 ...
 let save_useful_lines (filename : string) : unit =
    ...
    let str = Str.split (Str.regexp "[\n]") s in
  	...
  	for i=0 to (List.length str)-1 do
  		...
  		if (l="") || (l="(") || (l=")") || (String.starts_with l ";;") || (String.starts_with l "(then") || (String.starts_with l "(else") || (String.starts_with l "(local ") then 
  			(fprintf oc_1 "%d\n" (i+1))
  	done;
  	close_out oc_1;
    ...
    let ignore = ref false in 
   	for i=0 to (List.length str)-1 do
        ...
        if ((String.starts_with l "    (func $main") || (String.starts_with l "\t(func $main")) then (ignore := true) else (
          if ((String.starts_with l "    (func") || (String.starts_with l "\t(func")) then 
            (fprintf oc "%d-" (i+1))
          else (if (String.starts_with l "    )") || (String.starts_with l "\t)") then 
