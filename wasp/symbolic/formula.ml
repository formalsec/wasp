...
let rec to_string_aux (p : Symvalue.sym_expr -> string) (f : formula) : string =
  match f with
  | True -> "True"
  | False -> "False"
...
let to_string (f : formula) : string = to_string_aux Symvalue.to_string f
let pp_to_string (f : formula) : string = to_string_aux Symvalue.pp_to_string f
