...
let grow mem delta =
  ...
  else if not (within_limits new_size mem.max) then raise SizeLimit
  else
...
let memcpy (mem : memory) : memory =
  let new_array = create (size mem) in
  Array1.blit mem.content new_array;
  { content = new_array; max = mem.max }
...
let store_value mem a o v =
  let x =
    ...
    | F64 x -> F64.to_bits x
  in
  storen mem a o (Types.size (Values.type_of v)) x
...
let store_packed sz mem a o v =
  ...
  let n = packed_size sz in
  let x =
    match v with I32 x -> Int64.of_int32 x | I64 x -> x | _ -> raise Type
  in
  storen mem a o n x
