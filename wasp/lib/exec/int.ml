...
struct
   ...
   *)
  let divrem_u n d =
    if d = Rep.zero then raise Numeric_error.IntegerDivideByZero
    else
  ...
  let div_s x y =
    if y = Rep.zero then raise Numeric_error.IntegerDivideByZero
  ...
  let to_string_u i =
    if i >= Rep.zero then group_digits (Rep.to_string i)
    else group_digits (Rep.to_string (div_u i ten) ^ Rep.to_string (rem_u i ten))

  let rand upper =
    Random.self_init ();
    Rep.of_int (Random.int upper)
end
