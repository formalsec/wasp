...
let eval_binop (s1 : expr) (s2 : expr) (op : binop) : expr =
  ...
  let s =
    ...
    | Num c1, Num c2 -> Num (of_value (Eval_numeric.eval_binop op (to_value c1) (to_value c2)))
    | _ -> (* dispatch *)
        begin match op with
...
let eval_testop (e : expr) (op : testop) : expr =
  begin match e with
  | Num c -> Num (Num.num_of_bool (Interpreter.Eval_numeric.eval_testop op (to_value c)))
  | SymPtr (b, Num (I32 o)) ->
    let c = I32 (Int32.add b o) in
    Num (Num.num_of_bool (Interpreter.Eval_numeric.eval_testop op (to_value c)))
  | _ ->
      let open Interpreter in
      begin match op with
...
let eval_relop (s1 : expr) (s2 : expr) (op : relop) : expr =
  ...
  let s : expr =
    begin match s1, s2 with
    | Num c1, Num c2 -> Num (Num.num_of_bool (Interpreter.Eval_numeric.eval_relop op (to_value c1) (to_value c2)))
    | _ -> (* dispatch *)
        let open Interpreter in
        begin match op with
...
let eval_cvtop (op : cvtop) (s : expr) : expr =
  ...
  let s =
    begin match s with
    | Num c -> Num (of_value (Interpreter.Eval_numeric.eval_cvtop op (to_value c)))
    | _ ->
        (* dispatch cvtop func *)
        let open Interpreter in
        begin match op with
