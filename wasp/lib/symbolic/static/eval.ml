...
module SymbolicInterpreter (SM : Memory.SymbolicMemory) (E : Encoder) :
  ...
  let rec add_conds (encoder: E.t) (conds : t list) =
    match conds with
    | c :: [] -> E.add encoder c
    | c :: l  -> 
        E.add encoder c; add_conds encoder l
    | [] -> ()
  
  let config_load (config : sym_config) (vs' : t stack) (res : (SM.t * t * t list)) =
    let c, cf = clone config
    in
    let vs, es = cf.sym_code
    in
    match res with
    | (m, v, conds) -> add_conds cf.encoder conds;
    let es' = List.tl es in
    { cf with sym_code = (v :: vs', es') }
    ...
    | e :: t -> (
        ...
        | SPlain e', vs -> (
            ...
            | Load { offset; ty; sz; _ }, sym_ptr :: vs' -> (
                ...
                match res with
                | Ok mem_res -> 
                    Result.ok (Continuation (List.map (fun trio -> config_load c vs' trio) mem_res))
                | Error b -> 
                    let bug_type =
                      match b with
                ...
                Result.ok
                  (Continuation
                     [
                       {
                         c with
                         sym_code = (mk_symbol_s `I32Type x :: vs', es');
                       };
                     ])
            | GetSymInt64 x, vs' ->
                let es' = List.tl es in
                Result.ok
                  (Continuation
                     [
                       {
                         c with
                         sym_code = (mk_symbol_s `I64Type x :: vs', es');
                       };
                     ])
            | GetSymFloat32 x, vs' ->
                let es' = List.tl es in
                Result.ok
                  (Continuation
                     [
                       {
                         c with
                         sym_code = (mk_symbol_s `F32Type x :: vs', es');
                       };
                     ])
            | GetSymFloat64 x, vs' ->
                let es' = List.tl es in
                Result.ok
                  (Continuation
                     [
                       {
                         c with
                         sym_code = (mk_symbol_s `F64Type x :: vs', es');
                       };
                     ])
