...
let print_b_tree_keys (mem : memory) (n_keys : int) (max_keys : int)
  ...
  for i = 0 to n_keys - 1 do
    let v, sym =
      load_value mem (Int64.of_int (node_addr + 8 + (4 * i))) 0l I32Type
    in
...
let rec print_b_tree_level_order (n : int list) (mem : memory) (max_keys : int)
  ...
  for i = 0 to List.length n - 1 do
    ...
    let num_keys, _ =
      load_value mem (Int64.of_int (List.nth n i + 4)) 0l I32Type
    in
    print_b_tree_keys mem
      (int_of_string (string_of_value num_keys))
      max_keys (List.nth n i);
    ...
    if int_of_string (string_of_value leaf) = 0 then
      for j = 0 to int_of_string (string_of_value num_keys) do
        (* children *)
        (*let (value,_,_) = Hashtbl.find mem (((List.nth n i))+8+(4*max_keys)+(4*j)) in*)
        let value, _ =
          load_value mem
            (Int64.of_int (List.nth n i + 8 + (4 * max_keys) + (4 * j)))
            0l I32Type
        in
...
let print_b_tree (mem : memory) =
  ...
  let n_keys, _ =
    load_value mem
      (Int64.of_int (int_of_string (string_of_value root_addr) + 4))
      0l I32Type
  in
  print_b_tree_keys mem
    (int_of_string (string_of_value n_keys))
    max_keys
  ...
  let leaf, _ =
    load_value mem (Int64.of_string (string_of_value root_addr)) 0l I32Type
  in
  if int_of_string (string_of_value leaf) = 0 then
    for i = 0 to int_of_string (string_of_value n_keys) do
      (* children *)
