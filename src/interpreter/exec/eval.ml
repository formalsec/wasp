...
let rec step (c : config) : config =
  ...
  let vs', es' =
    ...
    | Plain e', vs -> (
      ...
      | Drop, v :: vs' -> (vs', [])
      | Select, I32 0l :: v2 :: v1 :: vs' -> (v2 :: vs', [])
      | Select, I32 i :: v2 :: v1 :: vs' -> (v1 :: vs', [])
      | LocalGet x, vs -> (!(local frame x) :: vs, [])
      | LocalSet x, v :: vs' ->
        local frame x := v;
        (* x-ésima variavel tem o valor v, avalia para o pointer daquela local *)
        ( vs'
        , []
          (* tira da stack e nao ha mais intruções depois --> este codigo vai ficar igual *)
        )
      | LocalTee x, v :: vs' ->
        local frame x := v;
        (v :: vs', [])
      ...
      | Load { offset; ty; sz; _ }, I32 i :: vs' -> (
        (* agr n interessa pq nao tamos a ver a heap*)
        Printf.printf "--- A LOAD WAS MADE ----\n";
      ...
      | Store { offset; sz; _ }, v :: I32 i :: vs' -> (
        (* agr n interessa pq nao tamos a ver a heap*)
        Printf.printf "--- A STORE WAS MADE ---\n";
        let mem = memory frame.inst (0l @@ e.at) in
        let addr = I64_convert.extend_i32_u i in
        try
          ( match sz with
