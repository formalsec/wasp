...
type op_type =
  { ins : infer_stack_type
  ; outs : infer_stack_type
  }
...
let check_memop (c : context) (memop : 'a memop) get_sz at =
  ...
  let size =
    ...
    | Some sz ->
      require
        (memop.ty = I64Type || sz <> Memory.Pack32)
        at "memory size too big";
...
let rec check_instr (c : context) (e : instr) (s : infer_stack_type) : op_type =
  ...
  | If (ts, es1, es2) ->
    ...
    [ I32Type ] --> ts
  | Br x -> label c x -->... []
  | BrIf x -> (label c x @ [ I32Type ]) --> label c x
  | BrTable (xs, x) ->
    let ts = label c x in
  ...
  | GlobalGet x ->
    let (GlobalType (t, _)) = global c x in
    [] --> [ t ]
  | GlobalSet x ->
  ...
  | Convert cvtop ->
    ...
    [ t1 ] --> [ t2 ]
  | Symbolic (t, _) -> [ I32Type ] --> [ t ]
  | Boolop _ -> [ I32Type; I32Type ] --> [ I32Type ]
  | Alloc -> [ I32Type; I32Type ] --> [ I32Type ]
  | Free -> [ I32Type ] --> []
  | SymInt32 _ -> [] --> [ I32Type ]
  | SymInt64 _ -> [] --> [ I64Type ]
  | SymFloat32 _ -> [] --> [ F32Type ]
  | SymFloat64 _ -> [] --> [ F64Type ]
  | SymAssume -> [ I32Type ] --> []
  | SymAssert -> [ I32Type ] --> []
  | Dup -> [] -~> [ peek 0 s ]
  | GetSymInt32 _ -> [] --> [ I32Type ]
  | GetSymInt64 _ -> [] --> [ I64Type ]
  | GetSymFloat32 _ -> [] --> [ F32Type ]
...
let check_value_type (_ : value_type) _ = ()
...
let check_table (_ : context) (tab : table) =
  let { ttype } = tab.it in
  check_table_type ttype tab.at

let check_memory (_ : context) (mem : memory) =
  let { mtype } = mem.it in
...
let check_start (c : context) (start : var option) =
  Lib.Option.app
    (fun x ->
      require
        (func c x = FuncType ([], []))
        x.at "start function must not have parameters or results" )
    start
...
let check_module (m : module_) =
  ...
  require
    (List.length c.tables <= 1)
    m.at "multiple tables are not allowed (yet)";
  require
