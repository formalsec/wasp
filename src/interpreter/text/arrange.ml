...
module IntOp = struct
  ...
  let testop _ = function Eqz -> "eqz"

  let relop _ = function
    | Eq -> "eq"
    | Ne -> "ne"
    | LtS -> "lt_s"
    | LtU -> "lt_u"
  ...
  let unop _ = function Clz -> "clz" | Ctz -> "ctz" | Popcnt -> "popcnt"
...
let extension = function Memory.SX -> "_s" | Memory.ZX -> "_u"
...
let rec instr e =
  let head, inner =
    ...
    | Binary op -> (binop op, [])
    | Convert op -> (cvtop op, [])
    | _ -> raise (Failure "operation not supported")
  in
  Node (head, inner)

let const c = list instr c.it
...
let table off i tab =
  ...
  Node ("table $" ^ nat (off + i) ^ " " ^ limits nat32 lim, [ atom elem_type t ])
...
let elems seg = segment "elem" (list (atom var)) seg

let data seg = segment "data" break_bytes seg
...
let assertion mode ass =
  ...
  | AssertMalformed (def, re) ->
    Node
      ("assert_malformed", [ definition `Original None def; Atom (string re) ])
